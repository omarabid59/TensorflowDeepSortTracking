...
class PredictorImage(AbstractPredictor):
    ...
    def predict_once(self,image_np):
        ...
        with self.sess.as_default(), self.detection_graph.as_default():
            ...
            if self.WITH_TRACKER:
                [boxes, scores, classes] = self.runTracker(boxes,scores,
                                                                classes, image_np.copy(),
                                                                self.score_thresh)
            else:
            ...
            # For now, we simply pass a reference to the image. We will use this for predicting on the subset.
    ...
    def inferNN(self, image_np):
        ...
        [boxes, scores, classes,temp_num_detections] = self.sess.run([self.boxes_tensor, self.scores_tensor, self.classes_tensor, self.num_detections_tensor],
